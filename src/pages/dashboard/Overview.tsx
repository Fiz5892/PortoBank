...
import { Copy, Eye, FolderOpen, Heart, MessageSquare } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="max-w-6xl space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {cards.map((card) => (
            <Card key={card.label} className="p-5 shadow-subtle">
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">{card.label}</p>
                <card.icon className="h-4 w-4 text-muted-foreground" />
              </div>
              <p className="font-heading text-3xl font-bold mt-2">{card.value}</p>
            </Card>
          ))}
        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Button asChild>
                <Link to="/dashboard/portfolio">
                  <FolderOpen className="mr-2 h-4 w-4" /> Manage Portfolio
                </Link>
              </Button>
            </div>
          </Card>
